...
def test_train_twisted_writes_checkpoint_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    config = TrainConfig(
        ...
        algo_kwargs={
            ...
            "transport_temperature": 0.5,
        },
    )

    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="twisted-smoke")
    ...
    assert "twisted_world_model_loss" in result.metrics
    assert "twisted_reuse_loss" in result.metrics
    assert "twisted_transport_weight_mean" in result.metrics
