...
def _register_tiny_render_env() -> str:
    env_id = "RLTrainingTest/CURLTrainerSmoke-v0"
    try:
        gym.spec(env_id)
...
def test_train_curl_writes_checkpoint_and_metrics(tmp_path: Path) -> None:
    config = TrainConfig(
        algo="curl",
    ...
    result = train_curl(config, run_suffix="smoke")
