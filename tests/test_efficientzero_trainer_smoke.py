...
def test_train_efficientzero_writes_checkpoint_and_consistency_metrics(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="efficientzero-smoke")
...
def test_train_efficientzero_supports_temperature_schedule(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(gym, "make", lambda env_id, **kwargs: DummyPixelEnv())
    ...
    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="efficientzero-temp-schedule")

    assert result.checkpoint_path is not None
    assert result.metrics["temperature"] == pytest.approx(0.15, rel=1e-6)


def test_train_efficientzero_supports_root_exploration_fraction_schedule(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(gym, "make", lambda env_id, **kwargs: DummyPixelEnv())
    ...
    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="efficientzero-root-noise-schedule")

    assert result.checkpoint_path is not None
    assert result.metrics["root_exploration_fraction"] == pytest.approx(0.1, rel=1e-6)


def test_train_efficientzero_supports_num_simulations_schedule(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.setattr(gym, "make", lambda env_id, **kwargs: DummyPixelEnv())
    ...
    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="efficientzero-sims-schedule")
