...
def test_train_diamond_writes_checkpoint_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="diamond-smoke")
    ...
    assert "diamond_world_model_loss" in result.metrics
    assert "diamond_denoising_loss" in result.metrics
    assert "diamond_noise_scale" in result.metrics
