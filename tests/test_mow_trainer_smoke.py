...
def test_train_mow_writes_checkpoint_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="mow-smoke")
    ...
    assert "mow_world_model_loss" in result.metrics
    assert "mow_gate_entropy" in result.metrics
    assert result.metrics["mow_num_experts"] == 3.0
