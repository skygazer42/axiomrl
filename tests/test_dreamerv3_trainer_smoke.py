...
def test_train_dreamerv3_writes_checkpoint_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="dreamerv3-smoke")
    ...
    assert "dreamerv3_world_model_loss" in result.metrics
    assert "dreamerv3_unimix_ratio" in result.metrics
