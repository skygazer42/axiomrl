...
def test_train_horizon_imagination_writes_checkpoint_and_metrics(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(gym, "make", lambda env_id, **kwargs: DummyPixelEnv())

    config = TrainConfig(
        algo="horizon_imagination",
        ...
        algo_kwargs={
            ...
            "gamma": 0.99,
            "entropy_coef": 1e-3,
            "denoising_loss_coef": 0.5,
            "noise_scale": 0.15,
            "stabilization_coef": 0.25,
            "schedule_bias": 0.5,
            "subframe_budget_ratio": 0.5,
        },
    )

    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="horizon-imagination-smoke")
    ...
    assert "horizon_imagination_world_model_loss" in result.metrics
    assert "horizon_imagination_stabilization_loss" in result.metrics
    assert "horizon_imagination_schedule_mean" in result.metrics
