...
class _StoppableTrainer(Protocol):
    global_step: int
...
class _StopAtGlobalStep(Callback):
    ...
    def on_collect_end(self, trainer: object, result: object) -> None:
        del result
        stoppable = cast(_StoppableTrainer, trainer)
        if stoppable.global_step >= self.target_step:
            stoppable.request_stop(f"stop at {self.target_step}")
