...
def _register_tiny_render_env() -> str:
    env_id = "RLTrainingTest/DrQv2TrainerSmoke-v0"
    try:
        gym.spec(env_id)
...
def test_train_drqv2_writes_checkpoint_and_metrics(tmp_path: Path) -> None:
    config = TrainConfig(
        algo="drqv2",
    ...
    result = train_drqv2(config, run_suffix="smoke")
