...
def test_train_po_dreamer_writes_checkpoint_and_metrics(tmp_path: Path, monkeypatch) -> None:
    ...
    config = TrainConfig(
        ...
        algo_kwargs={
            ...
            "memory_loss_coef": 0.5,
            "memory_mix": 0.35,
        },
    )

    result = get_algorithm_spec(config.algo).train_fn(config, run_suffix="po-dreamer-smoke")
    ...
    assert "po_dreamer_world_model_loss" in result.metrics
    assert "po_dreamer_memory_loss" in result.metrics
    assert "po_dreamer_memory_gate_mean" in result.metrics
