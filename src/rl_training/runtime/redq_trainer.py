...
from rl_training.algorithms.redq import REDQ
...
def _infer_spaces(envs: gym.vector.VectorEnv) -> tuple[int, int]:
    ...
    if not isinstance(obs_space, gym.spaces.Box):
        raise TypeError(f"unsupported observation space for REDQ trainer: {type(obs_space)!r}")
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for REDQ trainer: {type(action_space)!r}")
...
def _evaluate_redq_policy(
    ...
    *,
    ...
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for REDQ evaluation: {type(action_space)!r}")
...
def _maybe_update_algorithm(
    *,
    ...
    updated_metrics = latest_update_metrics
    ...
    for _ in range(gradient_updates_per_step):
        ...
        updated_metrics = result.metrics
        updated_count += result.num_gradient_steps
...
def _maybe_run_evaluation(
    *,
    ...
    eval_metrics = _evaluate_redq_policy(
        model,
        config,
        device=device,
...
def train_redq(
    ...
    *,
    ...
    session = create_training_session(config, algorithm="redq", run_suffix=run_suffix, callbacks=callbacks)
