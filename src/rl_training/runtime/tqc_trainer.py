...
from rl_training.algorithms.tqc import TQC
...
def _infer_spaces(envs: gym.vector.VectorEnv) -> tuple[int, int]:
    ...
    if not isinstance(obs_space, gym.spaces.Box):
        raise TypeError(f"unsupported observation space for TQC trainer: {type(obs_space)!r}")
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for TQC trainer: {type(action_space)!r}")
...
def _evaluate_tqc_policy(
    ...
    *,
    ...
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for TQC evaluation: {type(action_space)!r}")
...
def _maybe_run_evaluation(
    ...
    *,
    ...
    eval_metrics = _evaluate_tqc_policy(
        model,
        config,
        device=device,
...
def train_tqc(
    ...
    *,
    ...
    session = create_training_session(config, algorithm="tqc", run_suffix=run_suffix, callbacks=callbacks)
