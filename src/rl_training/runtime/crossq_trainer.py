...
from rl_training.algorithms.crossq import CrossQ
...
def _infer_spaces(envs: gym.vector.VectorEnv) -> tuple[int, int]:
    ...
    if not isinstance(obs_space, gym.spaces.Box):
        raise TypeError(f"unsupported observation space for CrossQ trainer: {type(obs_space)!r}")
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for CrossQ trainer: {type(action_space)!r}")
    if obs_space.shape is None or len(obs_space.shape) != 1:
        raise ValueError(f"CrossQ currently expects flat 1D observations, got shape={obs_space.shape!r}")
    if action_space.shape is None or len(action_space.shape) != 1:
        raise ValueError(f"CrossQ currently expects flat 1D actions, got shape={action_space.shape!r}")
...
def _evaluate_crossq_policy(
    ...
    *,
    ...
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for CrossQ evaluation: {type(action_space)!r}")
...
def _maybe_run_evaluation(
    ...
    *,
    ...
    eval_metrics = _evaluate_crossq_policy(
        model,
        config,
        device=device,
...
def train_crossq(
    ...
    *,
    ...
    session = create_training_session(config, algorithm="crossq", run_suffix=run_suffix, callbacks=callbacks)
