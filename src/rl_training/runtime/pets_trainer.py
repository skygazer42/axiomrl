...
def train_pets(
    ...
    *,
    ...
    if config.num_envs != 1:
        raise ValueError(f"pets trainer currently supports num_envs=1 only, got {config.num_envs}")
    ...
    try:
        ...
        while global_step < config.total_timesteps:
            if global_step < initial_random_steps or len(replay_buffer) < max(batch_size, learning_starts):
                action = _sample_random_action(action_space)
            else:
                action = algorithm.plan_action(
                    obs,
                    action_low=action_space.low,
