...
from rl_training.algorithms.curl import CURL
...
def _infer_spaces(envs: gym.vector.SyncVectorEnv) -> tuple[tuple[int, ...], int]:
    ...
    if not isinstance(obs_space, gym.spaces.Box):
        raise TypeError(f"unsupported observation space for CURL trainer: {type(obs_space)!r}")
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for CURL trainer: {type(action_space)!r}")
    ...
    *,
    ...
    class _ActionFn:
        ...
        def bind_env(self, env: gym.Env) -> None:
            ...
            if not isinstance(action_space, gym.spaces.Box):
                raise TypeError(f"unsupported action space for CURL evaluation: {type(action_space)!r}")
...
def train_curl(
    ...
    *,
    ...
    session = create_training_session(config, algorithm="curl", run_suffix=run_suffix, callbacks=callbacks)
