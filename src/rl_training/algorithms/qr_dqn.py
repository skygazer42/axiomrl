...
def _quantile_huber_loss(
    ...
    *,
    ...
    per_sample = (quantile_weights * huber).mean(dim=(1, 2))
    return per_sample


def qr_loss(batch: dict[str, torch.Tensor]) -> dict[str, float]:
...
class QRDQN:
    ...
    def register_buffer(self, name: str, tensor: torch.Tensor) -> None:
        # keep the algorithm torchscript-like without being an nn.Module
    ...
    def update(self, batch: dict[str, Any], *, global_step: int) -> UpdateResult:
        ...
        with torch.no_grad():
            next_online_quantiles = self.q_network(next_obs)
            next_actions = next_online_quantiles.mean(dim=-1).argmax(dim=-1)
            next_target_quantiles_all = self.target_network(next_obs)
