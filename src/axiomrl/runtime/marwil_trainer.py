...
from rl_training.algorithms.marwil import MARWIL
...
def train_marwil(
    ...
    *,
    ...
    session = create_training_session(config, algorithm="marwil", run_suffix=run_suffix, callbacks=callbacks)
    ...
    returns_to_go_gamma = float(config.algo_kwargs.get("returns_to_go_gamma", gamma))
    ...
    try:
        ...
        dataset = _build_offline_dataset(config, action_space=action_space).with_discounted_returns_to_go(
            gamma=returns_to_go_gamma
        )
        ...
        algorithm = MARWIL(
            ...
            beta=beta,
            vf_coeff=vf_coeff,
            moving_average_sqd_adv_norm_start=moving_average_sqd_adv_norm_start,
            moving_average_sqd_adv_norm_update_rate=moving_average_sqd_adv_norm_update_rate,
        )
