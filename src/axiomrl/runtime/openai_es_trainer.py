...
from rl_training.algorithms.openai_es import OpenAIES
...
def _infer_spaces(env: gym.Env) -> tuple[int, int]:
    ...
    if not isinstance(obs_space, gym.spaces.Box):
        raise TypeError(f"unsupported observation space for OpenAI ES trainer: {type(obs_space)!r}")
    if not isinstance(action_space, gym.spaces.Box):
        raise TypeError(f"unsupported action space for OpenAI ES trainer: {type(action_space)!r}")
...
def _evaluate_openai_es_policy(
    model: MLPARSModel,
    config: TrainConfig,
    *,
    device: torch.device,
    ...
    try:
        ...
        while global_step < config.total_timesteps:
            ...
            if should_run_evaluation(
                ...
                eval_metrics = _evaluate_openai_es_policy(
                    model,
                    config,
                    device=device,
