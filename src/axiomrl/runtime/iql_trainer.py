    ...
    class _ActionFn:
        ...
        def __call__(self, obs_tensor: torch.Tensor) -> np.ndarray:
            ...
            with torch.no_grad():
                ...
                env_action = _scale_actions(normalized_action, low=self.low, high=self.high).squeeze(0)
...
def train_iql(
    ...
    *,
    ...
    session = create_training_session(config, algorithm="iql", run_suffix=run_suffix, callbacks=callbacks)
