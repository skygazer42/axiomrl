...
def _cql_loss_terms(batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    ...
    cql_penalty = batch["cql_alpha"] * (batch["cql_penalty_q1"] + batch["cql_penalty_q2"])
...
def cql_loss(batch: dict[str, torch.Tensor | float]) -> dict[str, float]:
    tensor_batch = {
        ...
        "cql_penalty_q1": torch.as_tensor(batch["cql_penalty_q1"], dtype=torch.float32),
        "cql_penalty_q2": torch.as_tensor(batch["cql_penalty_q2"], dtype=torch.float32),
        "cql_alpha": torch.as_tensor(batch["cql_alpha"], dtype=torch.float32),
    }
    terms = _cql_loss_terms(tensor_batch)
...
class CQL:
    ...
    def _cql_penalty(
        ...
        conservative_q1 = torch.logsumexp(torch.cat([random_q1, sampled_q1], dim=1), dim=1).mean()
        conservative_q2 = torch.logsumexp(torch.cat([random_q2, sampled_q2], dim=1), dim=1).mean()
        return conservative_q1 - current_q1.mean(), conservative_q2 - current_q2.mean()
    ...
    def update(self, batch: dict[str, Any], *, global_step: int) -> UpdateResult:
        ...
        cql_penalty_q1, cql_penalty_q2 = self._cql_penalty(obs, current_q1, current_q2)
        self.critic_optimizer.zero_grad(set_to_none=True)
        critic_terms = _cql_loss_terms(
            {
