...
class MLPCrossQModel(nn.Module):
    def __init__(
        self,
        *,
        obs_dim: int,
        ...
        critic_hidden_sizes: Sequence[int] = (512, 512),
        ...
        self.actor_log_std = nn.Linear(hidden_sizes[-1], action_dim)

        critic_input_dim = obs_dim + action_dim
        # PyTorch does not ship Batch Renorm, so v1 uses BatchNorm1d as the
        # practical normalization layer for CrossQ-style critics.
        self.q1 = _build_batchnorm_mlp(
    ...
    def sample_actions(self, obs: object, *, deterministic: bool = False) -> CrossQSample:
    ...
    def set_critic_bn_training_mode(self, training: bool) -> None:
        for module in list(self.q1.modules()) + list(self.q2.modules()):
