...
class MLPTQCModel(nn.Module):
    ...
    def sample_actions(self, obs: object, *, deterministic: bool = False) -> TQCSample:
    ...
    def quantile_values(self, obs: object, actions: object) -> torch.Tensor:
        obs_tensor = self._prepare_obs(obs)
        ...
        quantiles = [critic(inputs) for critic in self.critics]
        return torch.stack(quantiles, dim=1)

    def expected_q_values(self, obs: object, actions: object) -> torch.Tensor:
        quantiles = self.quantile_values(obs, actions)
        return quantiles.mean(dim=-1).mean(dim=-1)
