...
class CNNPPGModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.backbone, head_dim = _build_backbone(
            input_dim=int(features_dim),
            hidden_sizes=hidden_sizes,
            activation=activation,
        )
        self.policy_head = nn.Linear(head_dim, self.action_dim)
        self.value_head = nn.Linear(head_dim, 1)
        self.auxiliary_value_head = nn.Linear(head_dim, 1)
