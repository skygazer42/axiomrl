...
```
axiomrl              # 根包（稳定核心 + 弃用兼容）
axiomrl.core         # 稳定核心 API
axiomrl.experimental # 实验性 API
axiomrl.contrib      # 社区扩展
```

### axiomrl（根包）

根包通过延迟导入转发稳定核心 API 的所有名称。对于不在稳定核心中的算法名称，根包会发出 `DeprecationWarning` 并从 `axiomrl.api` 中回退加载。
...
# 稳定名称 - 直接可用
algo = axiomrl.PPO
config = axiomrl.TrainConfig

# 实验性名称 - 触发弃用警告
# algo = axiomrl.SomeExperimentalAlgo  # DeprecationWarning
```
...
```python
from axiomrl.core import PPO, SAC, TrainConfig, STABLE_ALGORITHMS
```

### axiomrl.experimental（实验性）
